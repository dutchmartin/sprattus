...
) -> Box<dyn Future<Item = Vec<Product>, Error = error::Error> + Send> {
    let fut = client_ref
        .lock()
        .unwrap()
        .prepare("SELECT * FROM products ORDER BY prod_id DESC limit 5");
    let ret_val = fut
        .and_then(move |statement| client_ref.lock().unwrap().query(&statement, &[]).collect())
        .map(move |rows| {
            let mut n: usize = 0;
            rows.iter()
                .map(move |row| {
...
fn get_client() -> Box<dyn Future<Item = Client, Error = error::Error> + Send> {
    Box::new(
        tokio_postgres::connect("postgresql://localhost/dellstore2?user=tg", NoTls).map(
            |(client, connection)| {
