...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let connection = connection.map_err(|e| eprintln!("connection error: {}", e)).map(|result|result.unwrap());
...
async fn get_products(client_ref: Arc<Mutex<Client>>, statement: Statement) -> Result<Vec<Product>, tokio_postgres::Error> {
    client_ref
        .lock()
        .unwrap()
